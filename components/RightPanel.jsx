...
export default function RightPanel(){
  ...
  async function runAnalyze(){
    setLoading(true)
    try{
      const res = await fetch('/api/ai/analyze', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ notes:'Run roster & risk analysis' }) })
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      ...
      <div style={{fontSize:13,color:'var(--muted)'}}>KPI Summary</div>
      {kpis ? (
